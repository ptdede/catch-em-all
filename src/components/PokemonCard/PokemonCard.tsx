    ...
    <TPokemonCard.Wrapper>
      <Link href={href}>
        <TPokemonCard.Inner color={colors.dominant}>
          {!hideOwned && (
            <TPokemonCard.Owned bold={owned > 0}>
              {owned > 0 ? `owned: ${owned}` : "not owned"}
            </TPokemonCard.Owned>
          )}
          <TPokemonCard.Image src={image} alt={name} />
          <TPokemonCard.Name>{name}</TPokemonCard.Name>
        </TPokemonCard.Inner>
      </Link>
      {enableRelease && (
        <TPokemonCard.ActionWrapper onClick={() => releasePokemon(id)}>
          <p>Release!</p>
